...
public class ChatController {
    ...
    public ResponseEntity<ApiResponse<ChatResponse>> chat(@RequestBody ChatRequest request) {
        try {
            ...
            // 获取实际的 sessionId（如果请求为空，这里是生成的 UUID）
            String actualSessionId = session.getSessionId();

            return ResponseEntity.ok(ApiResponse.success(ChatResponse.success(fullAnswer, actualSessionId)));
    ...
    public ResponseEntity<ApiResponse<SessionInfoResponse>> getSessionInfo(@PathVariable String sessionId) {
        try {
            ...
            // 内存没有，查 Redis
            com.spf.dto.ChatSession redisSession = chatSessionService.getOrCreateSession(sessionId);
    ...
    public ResponseEntity<ApiResponse<SessionListResponse>> listSessions(
        ...
        try {
            logger.info("获取会话列表 - page: {}, pageSize: {}", page, pageSize);
    ...
    public ResponseEntity<ApiResponse<List<ChatMessage>>> getSessionMessages(
        ...
        try {
            logger.info("获取会话消息历史 - SessionId: {}, Limit: {}", sessionId, limit);
    ...
    private SessionInfo getOrCreateSession(String sessionId) {
        ...
        if (!recovered.getRecentMessages().isEmpty()) {
            ...
            for (ChatMessage msg : recovered.getRecentMessages()) {
                ...
                sessionInfo.messageHistory.add(map);
            }
        }
    ...
    public static class ChatResponse {
        ...
        private String sessionId;  // 会话ID，用于前端维护多轮对话
